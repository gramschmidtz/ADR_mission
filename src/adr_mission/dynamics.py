...
from .utils import mee_to_eci, make_unit_vector, P2sinphi

def make_rho(h, rho0, h0, H):
    return rho0 * torch.exp(-(h-h0)/(H))
...
class Spacecraft(SpaceObject):
    """
    SpaceObject를 상속받은 능동 제어 위성(Chaser)
    추력(a_T) 입력 적용 및 질량 감소(연료 소모) 로직 추가
    """
    def __init__(self, vehicle_cfg, constants_cfg):
        super().__init__(vehicle_cfg, constants_cfg)
        self.vehicle_cfg = vehicle_cfg
        self.I_sp = float(vehicle_cfg['propulsion']['I_sp_s'])
        self.T_max = float(vehicle_cfg['propulsion']['T_max_N'])

    def dynamics_model(self, x: torch.Tensor, u: torch.Tensor, t: float) -> torch.Tensor:
        """
        Spacecraft의 동역학 모델. (Override)
        a_total = a_T(u) + a_g + a_D
        u는 (N_r, N_theta, N_h) 형태의 제어 입력 (추력 가속도)
        """
        state = MEEState.from_tensor(x)
        ...
        # 궤도 요소 변화율 계산
        x_dot = torch.bmm(A, a_total.unsqueeze(-1)) + b

        # 연료 소모에 의한 질량 변화율 계산 (식 10)
        N_norm = torch.linalg.norm(u, ord=2, dim=1, keepdim=True)
        mass_dot = - ((self.T_max * N_norm) / (self.I_sp * self.g_e))
