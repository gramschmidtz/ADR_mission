...
### 8.1 내부 truth state는 MEE를 사용한다
이 프로젝트의 내부 propagation state는 논문과 동일하게 **Modified Equinoctial Elements (MEE)** 를 사용한다.
...
여기서
- `p`: semilatus rectum
- `f, g`: eccentricity-related elements
- `h, k`: inclination-related elements
- `L`: true longitude
- `m`: spacecraft mass

즉, 전파기의 source of truth는 MEE이며,
사용자가 어떤 형식으로 초기 상태를 주더라도 propagation 직전에는 반드시 MEE로 정규화된다.

### 8.2 외부 입력 state는 세 가지를 허용한다
사용자는 초기 상태를 아래 세 형식 중 하나로 줄 수 있다.

```text
1. MEE
2. Cartesian coordinates
3. Keplerian elements
```

하지만 내부에서는 모두 아래 흐름으로 처리한다.

```text
input state -> normalize/convert -> MEE propagation state
```
...
### 8.3 Cartesian 입력을 위해서는 변환 유틸이 필요하다
초기 상태를 Cartesian으로 주는 경우, 다음 변환 유틸이 필요하다.

- `cartesian_to_mee()`
- `mee_to_cartesian()`

시각화는 여전히 Cartesian이 더 직관적이므로, propagation은 MEE에서 수행하되 export 시 Cartesian history를 함께 저장하는 것이 좋다.

### 8.4 Keplerian elements는 항상 완전 정의되지 않는다
Keplerian elements는 원궤도 및 적도궤도에서 각 요소가 특이해질 수 있다.

대표적으로:
- 원궤도에서는 `omega`(argument of periapsis)가 정의되지 않는다.
- 적도궤도에서는 `Omega`(RAAN)가 정의되지 않는다.
- 원궤도 + 적도궤도에서는 `Omega`, `omega`가 동시에 의미를 잃으며, 경우에 따라 `nu`도 단독 해석보다 `true longitude`로 다루는 편이 자연스럽다.

따라서 이 프로젝트에서는 Keplerian state를 단순 숫자 묶음으로만 취급하지 않고,
**값과 정의 여부를 함께 관리**한다.

### 8.5 정의되지 않는 각도는 None으로 반환한다
Keplerian 변환 결과에서 정의되지 않는 각도는 임의의 숫자로 억지 지정하지 않고 `None`으로 반환한다.

예:
- circular inclined orbit: `argp = None`, 대신 `argument_of_latitude` 사용
- equatorial elliptical orbit: `raan = None`, 대신 `longitude_of_periapsis` 사용
- circular equatorial orbit: `raan = None`, `argp = None`, 대신 `true_longitude` 사용

필요하면 내부 계산 편의를 위해 substitute angle을 둘 수 있지만, 사용자에게 노출되는 상태에서는 물리적으로 정의된 값과 구분해서 표시한다.

### 8.6 Keplerian state는 보조각을 함께 가진다
특이 케이스를 안정적으로 표현하기 위해 Keplerian state는 아래 보조각을 함께 가질 수 있어야 한다.
...
만으로 끝나는 것이 아니라, singularity metadata와 auxiliary angles를 포함하는 구조여야 한다.

### 8.7 singularity policy를 고정한다
원궤도/적도궤도 판단은 tolerance 기반으로 고정한다.

예:
- `e < ECC_EPS` -> circular
- `sin(i) < INC_EPS` 또는 `i < INC_EPS` -> equatorial

이 기준은 변환 함수 전체에서 일관되게 사용해야 한다.

---

## 9. 현재 단계에서의 권장 구현 우선순위

## 단계 1: 상태 표현과 변환 계층
- `MEEState` 정의
- `CartesianState` 정의
- `KeplerianState` 정의
- singularity metadata 정의
- `Cartesian <-> MEE` 변환
- `Keplerian <-> MEE` 변환

## 단계 2: 최소 동작 MEE dynamics + propagation + MATLAB visualization
- MEE state 정의
- 논문 식의 `eom_mee.py` 구현
...
## 단계 4: perturbation 및 richer control
- drag
- J2
...
## 10. 현재 단계에서의 기술적 권장사항

## 10.1 첫 working EOM은 MEE로 구현한다
이 프로젝트의 첫 working EOM은 Cartesian이 아니라 **논문과 동일한 MEE dynamics**로 구현한다.
...
이 선택의 이유는 다음과 같다.
- 논문 구현과 직접 대응된다.
- 원궤도/저이심률 영역에서 classical Keplerian singularity를 피할 수 있다.
- 이후 transfer model, RAAN-phasing, 학습 데이터 생성으로 이어질 때 상태 표현의 일관성이 유지된다.

단, MEE는 사람이 직관적으로 이해하기 어렵기 때문에
- 입력은 Cartesian/Keplerian/MEE를 모두 허용하고
- 출력과 시각화는 Cartesian history도 함께 제공한다.

---

## 10.2 Cartesian과 Keplerian 입력은 어댑터 계층에서 MEE로 변환한다
현재 subgoal의 핵심 문장은 “초기 상태와 입력 프로필이 주어졌을 때”이다.
따라서 입력 상태 형식과 propagation state는 분리해야 한다.

권장 흐름:
```text
user input (Cartesian / Keplerian / MEE)
    -> input adapter
    -> normalized MEE state
    -> propagation
```

즉 state 변환 유틸은 dynamics보다 앞단의 필수 모듈이다.

---

## 10.3 control profile은 독립 모듈로 둔다
현재 subgoal의 핵심 문장은 “초기 상태와 입력 프로필이 주어졌을 때”이다.  
...
## 10.4 export schema는 초반부터 고정 철학을 가진다
...
현재 단계에서 가장 먼저 만드는 파일은 아래 세 개다.

- `src/adr_mission/orbit/state_definitions.py`
- `src/adr_mission/orbit/conversions.py`
- `src/adr_mission/orbit/eom_mee.py`

이 순서를 택하는 이유는 다음과 같다.

1. MEE propagation을 하려면 먼저 내부 state 정의가 고정되어야 한다.
2. Cartesian/Keplerian 입력을 받으려면 MEE로 바꾸는 변환층이 먼저 필요하다.
3. 그 다음에야 논문 식의 MEE dynamics를 안정적으로 구현할 수 있다.

즉, 현재 가장 먼저 만드는 것은 단순한 propagator가 아니라 **state 표현과 변환 계층**이다.
