...
def main():
    ...
    # MEE 초기값
    initial_mee_state = ke_to_mee(ke_init)
    ...
    debris_logger = ObjectTrajectory(num_steps, batch_size, 7, device)
    debris_state = initial_mee_state
    ...
    if is_chaser_on:
        ...
        chaser_logger = ObjectTrajectory(num_steps, batch_size, 7, device)
        chaser_state = initial_mee_state
    ...
    for step in tqdm(range(num_steps), desc="Simulating Orbit"):
        ...
        debris_state = rk4(debris.dynamics_model, debris_state, u_zero, t, dt)
